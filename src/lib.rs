...
impl AssetLoader for KenneySpriteSheetAssetLoader {
    ...
    async fn load(
        ...
        let sheet_dimensions = UVec2::new(spritesheet_size.x, spritesheet_size.y);

        let mut layout = TextureAtlasLayout::new_empty(sheet_dimensions);
        let sub_textures = doc
            ...
            .map(|tex| {
                ...
                let width: u32 = tex.attribute("width")?.parse().ok()?;
                let height: u32 = tex.attribute("height")?.parse().ok()?;

                layout.add_texture(URect::from_corners(
                    UVec2::new(x, y),
                    UVec2::new(x + width, y + height),
                ));
                Some(SubTexture {
                    name: tex.attribute("name")?.to_string(),
                    x,
                    y,
                    width,
                    height,
            ...
            .collect::<Option<Vec<SubTexture>>>()
            .ok_or(KenneySpriteSheetAssetLoaderError::InvalidSubTexture)?;
